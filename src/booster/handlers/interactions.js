...
export async function handleBoosterInteraction(interaction, client) {
  ...
  if (id === 'booster_restore') {
    ...
    return interaction.reply({
      embeds: [new EmbedBuilder().setColor(0x57F287).setTitle('♻️ Restore')
  ...
  if (id === 'booster_share_list') {
    ...
    return interaction.reply({
      embeds: [new EmbedBuilder().setColor(0x5865F2).setTitle('👥 Sharing')
        .setDescription(role?.sharedWith.length ? role.sharedWith.map(i => `<@${i}>`).join(', ') : 'Nobody added yet.')],
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  if (id === 'booster_templates') {
    ...
    return interaction.reply({
      embeds: [new EmbedBuilder().setColor(0xF47FFF).setTitle('🎭 Templates')
        .setDescription(tmpl.map(t => `${t.emoji} **${t.name}** — \`${t.color}\``).join('\n') || 'None.')
        .setFooter({ text: 'Apply: .booster template apply <name>' })],
      flags: MessageFlags.Ephemeral,
    });
  }
  ...
  if (id === 'booster_req_vc') {
    ...
    return interaction.showModal(
      new ModalBuilder().setCustomId('booster_vc_modal').setTitle('🔊 Custom Voice Channel').addComponents(
