...
async function panel(message) {
  ...
  const embed = new EmbedBuilder().setColor(0x5865F2).setTitle('⚙️ Booster System Settings')
    .addFields(
      ...
      { name: 'Role Boundaries', value: settings.boundaries.upperRoleId
          ? `Upper: <@&${settings.boundaries.upperRoleId}> | Lower: <@&${settings.boundaries.lowerRoleId}>`
          : '⚠️ Not configured — all positions allowed', inline: false },
  ...
  const mkBtn = k => new ButtonBuilder()
    .setCustomId(`bsettings_toggle_${k}`)
    .setLabel(FEATURE_LABELS[k].replace(/^\S+\s/, ''))
    .setStyle(settings.features[k] !== false ? ButtonStyle.Success : ButtonStyle.Danger);

  const row1 = new ActionRowBuilder().addComponents(featureKeys.slice(0, 5).map(mkBtn));
  const row2 = new ActionRowBuilder().addComponents(featureKeys.slice(5).map(mkBtn));
...
async function boundaries(message, args) {
  ...
  if (sub === 'set') {
    ...
    const settings = await getSettings(message.guild.id);
    settings.boundaries.upperRoleId = upperRole.id;
    settings.boundaries.lowerRoleId = lowerRole.id;
    await settings.save();
  ...
  return message.channel.send({ embeds: [new EmbedBuilder().setColor(0x5865F2).setTitle('📏 Role Boundaries')
    .setDescription(settings.boundaries.upperRoleId
      ? `**Upper:** <@&${settings.boundaries.upperRoleId}>\n**Lower:** <@&${settings.boundaries.lowerRoleId}>`
      : '⚠️ No boundaries set. Use `.settings boundaries set @upper @lower`.')] });
}
...
async function templates(message, args) {
  ...
  if (sub === 'list') {
    ...
    return message.channel.send({ embeds: [new EmbedBuilder().setColor(0xF47FFF).setTitle('🎭 Templates')
      .setDescription(list.map(t => `${t.emoji} **${t.name}** — \`${t.color}\` ${t.builtIn ? '*(built-in)*' : '*(custom)*'}`).join('\n') || 'None.')] });
  }
  if (sub === 'add') {
